...
const form = reactive({
    id: props.total[0].id,
    date: dayjs(props.total[0].created_at).format('YYYY-MM-DD HH:mm:ss'),
    customer_id: props.total[0].customer_id,
    status: props.total[0].status,
    items: []
})
                                ...
                                <div class="container px-5 py-8 mx-auto">
                                    <div class="lg:w-1/2 md:w-2/3 mx-auto">
                                        <div class="flex flex-wrap -m-2">
                                            ...
                                            <div class="p-2 w-full">
                                                <div class="relative">
                                                    <label for="customer" class="leading-7 text-sm text-gray-600">会員氏名</label>
                                                    <input disabled
                                                        type="text" id="customer" name="customer"
                                                        :value="props.total[0].customer_name"
                                                        class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
                                                </div>
                                            </div>
                                            ...
                                            <div class="p-2 w-full">
                                                <div class="relative">
                                                    ...
                                                    <input 
                                                        type="radio" id="status" name="status" v-model="form.status"
                                                        value="1" >未キャンセル
                                                    <input 
                                                        type="radio" id="status" name="status" v-model="form.status"
                                                        value="0" >キャンセルする
